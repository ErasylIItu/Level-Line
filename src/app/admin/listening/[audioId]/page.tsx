...
import type { BaseQuestion, ListeningAudio, QuestionOption } from "@/types";

const TYPE = "listening" as const;
...
export default function AudioQuestionsPage({
  ...
  const [audio, setAudio] = useState<ListeningAudio | null>(null);
  ...
  useEffect(() => {
    fetchAudios()
      .then((audios) => {
        const found = audios.find((a) => a.id === audioId) ?? null;
        setAudio(found);
        setNotFound(!found);
  ...
  async function handleCreate(input: {
    ...
    const created = await createQuestion(TYPE, input, { audioId });
  ...
  return (
    <div>
      ...
      <AdminPageHeader
        title={audio?.title ?? (loading ? "Loading..." : "Audio not found")}
        description="Manage the questions that belong to this audio."
      />
